...
public class OnevsoneListener implements Listener
{
    ...
    public void playerInteractEvent(final PlayerInteractEntityEvent event) {
        ...
        if (!player.getItemInHand().equals((Object)OnevsoneManager.blaze_rod)) {
            return;
        }
        ...
        if (OnevsoneManager.invite.contains(player.getUniqueId()) && OnevsoneManager.invited.contains(target.getUniqueId())) {
        ...
        }.runTaskLater((Plugin)Main.getInstance(), 250L);
    }
    ...
    public void playerInteract(final PlayerInteractEvent event) {
        ...
        if (player.getItemInHand().equals((Object)OnevsoneManager.gray_color)) {
            ...
            if (OnevsoneManager.fast.size() == 2) {
                ...
                new BukkitRunnable() {
                    public void run() {
                        if (OnevsoneManager.fast.contains(player) && player.getInventory().getItem(5).equals((Object)OnevsoneManager.green_color)) {
            ...
            if (!player.getItemInHand().equals((Object)OnevsoneManager.green_color)) {
                return;
            }
            if (!OnevsoneManager.fast.contains(player)) {
                return;
    ...
    public void processocommand(final PlayerCommandPreprocessEvent e) {
        ...
        if (OnevsoneManager.inWarp.contains(e.getPlayer().getUniqueId()) && OnevsoneManager.fighting.containsKey(p.getUniqueId())) {
