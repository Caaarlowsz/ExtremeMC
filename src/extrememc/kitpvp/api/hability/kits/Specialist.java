...
public class Specialist implements Listener
{
    ...
    public void onIasnteract(final PlayerInteractEvent event) {
        ...
        if (KitAPI.hasKit(p, Kits.SPECIALIST) && (event.getAction() == Action.RIGHT_CLICK_AIR || event.getAction() == Action.RIGHT_CLICK_BLOCK || event.getAction() == Action.LEFT_CLICK_AIR || event.getAction() == Action.LEFT_CLICK_BLOCK) && p.getItemInHand().getType() == Material.BOOK) {
