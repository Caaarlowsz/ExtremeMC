...
public class KitAPI
{
    ...
    public static void giveItens(final Player player) {
        ...
        for (int i = 0; i < 34; ++i) {
            inv.addItem(new ItemStack[] { ItemBuilder.criarItem(Material.MUSHROOM_SOUP) });
        }
    ...
    public static void giveKit(final Player player, final Kits kit) {
        ...
        if (kit.equals(Kits.FISHERMAN)) {
            inv.setItem(1, ItemBuilder.criarItem(Material.FISHING_ROD, String.valueOf(Strings.primaryColor) + "Fisherman"));
            player.updateInventory();
        }
        ...
        if (kit.equals(Kits.SWITCHER)) {
            inv.setItem(1, ItemBuilder.criarItem(Material.SNOW_BALL, String.valueOf(Strings.primaryColor) + "Switcher", 16));
            player.updateInventory();
        }
        if (kit.equals(Kits.GLADIATOR)) {
            inv.setItem(1, ItemBuilder.criarItem(Material.IRON_FENCE, String.valueOf(Strings.primaryColor) + "Gladiator"));
        ...
        if (kit.equals(Kits.GRANDPA)) {
            inv.setItem(1, ItemBuilder.criarItem(Material.STICK, String.valueOf(Strings.primaryColor) + "Grandpa", Enchantment.KNOCKBACK, 2));
            player.updateInventory();
        }
    ...
    public static boolean hasKitPermission(final Player player, final Kits kit) {
        ...
        return kit.equals(Kits.ANTISTOMPER) && (Main.getInstance().getPermissions().isBeta(player) || extrememc.kitpvp.Main.fullkit);
    }
