...
public class MorreuListener implements Listener
{
    ...
    public void asd(final PlayerDeathEvent event) {
        ...
        if (event.getEntity() instanceof Player) {
            ...
            if (WarpsAPI.isInWarp(morreu, WarpsAPI.Warps.ONEVSONE)) {
                ...
                final int sopasKiller = this.itemsInInventory((Inventory)matou.getInventory(), new Material[] { Material.MUSHROOM_SOUP });
                morreu.sendMessage("§c§lMORREU §fVoc\u00ea morreu para o jogador " + matou.getName() + " e ele ficou com " + sopasKiller + " §fsopas restantes.");
            ...
            if (event.getEntity().getKiller() instanceof Player) {
                ...
                if (WarpsAPI.isInWarp(matou, WarpsAPI.Warps.ONEVSONE)) {
                    final int sopasEntity = this.itemsInInventory((Inventory)morreu.getInventory(), new Material[] { Material.MUSHROOM_SOUP });
                    matou.sendMessage("§a§lMATOU §fVoc\u00ea matou o jogador §a" + morreu.getName() + " §fe ele ficou com §a" + sopasEntity + " §fsopas restantes.");
                ...
                new BukkitRunnable() {
                    public void run() {
                        if (jogador3.getStatusKitPvP().getStreak() % 5 == 0) {
                            Bukkit.broadcastMessage("§e§lKILLSTREAK §fO jogador " + matou.getName() + " est\u00e1 com um §a§lSTREAK §fde " + Strings.primaryColor + jogador3.getStatusKitPvP().getStreak());
                        }
                    }
    ...
    public void playerQuitEvent(final PlayerQuitEvent event) {
        ...
        if (OnevsoneManager.fighting.containsKey(player.getUniqueId())) {
            ...
            jogador2.getStatusKitPvP().save();
            jogador2.getStatusGlobal().save();
            final int sopasEntity = this.itemsInInventory((Inventory)player.getInventory(), new Material[] { Material.MUSHROOM_SOUP });
            if (WarpsAPI.isInWarp(killer, WarpsAPI.Warps.ONEVSONE)) {
                killer.sendMessage("§a§lMATOU §fVoc\u00ea matou o jogador §a" + player.getName() + " §fe ele ficou com §a" + sopasEntity + " §fsopas restantes.");
                if (percentKillerXP == 0) {
